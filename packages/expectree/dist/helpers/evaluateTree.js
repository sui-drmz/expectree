...
import { isBinaryNode, isUnaryNode } from '../tree/types';
/**
 ...
 */
export function evaluateTree(root, statusMap) {
 ...
 */
const createEvaluator = (statusMap) => {
    return (node) => {
        switch (node.type) {
            case 'AND':
                return evaluateAnd(node, statusMap);
            case 'OR':
                return evaluateOr(node, statusMap);
            case 'NOT':
                return evaluateNot(node, statusMap);
            case 'GROUP':
                return evaluateGroup(node, statusMap);
            case 'EXPECTATION':
                return evaluateExpectation(node, statusMap);
            case 'ROOT':
                return evaluateRoot(node, statusMap);
            default:
 ...
 * @returns The root node's snapshot.
 */
const evaluateRoot = (root, statusMap) => {
    if (!root.child)
        return { type: 'ROOT', status: 'PENDING' };
 ...
 */
const evaluateAnd = (node, statusMap) => {
    const evaluate = createEvaluator(statusMap);
 ...
 */
const evaluateOr = (node, statusMap) => {
    const evaluate = createEvaluator(statusMap);
    const left = evaluate(node.left);
 ...
 */
const evaluateNot = (node, statusMap) => {
    const evaluate = createEvaluator(statusMap);
 ...
 */
const evaluateGroup = (node, statusMap) => {
    const evaluate = createEvaluator(statusMap);
    const child = evaluate(node.child);
    return {
        type: 'GROUP',
 ...
 */
const evaluateExpectation = (node, statusMap) => {
    const status = statusMap.get(node.id);
    if (!status)
        throw new Error(`Status missing for expectation ${node.id}`);
    return {
        type: 'EXPECTATION',
        id: node.id,
        status,
 ...
 */
const combineAndStatus = (left, right) => {
    if (right === 'FAILED')
        return 'FAILED';
 ...
 * @returns the combined status
 */
const combineOrStatus = (left, right) => {
    if (right === 'PASSED')
        return 'PASSED';
